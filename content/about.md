+++
title = "About"
layout = "about"
hidden = true
type = "page"
summary = "我是谁？"
url = "/about/"

+++

{{< avatar >}}

## Who am I

Welcome to my theme—I’m Tajang.


---------------------------------

## Something

I’m a script kiddie who likes to write articles in my spare time.

Hugo boasts exceptional performance, but I couldn’t find a theme I really liked on theme directories, so I decided to roll up my sleeves and build one myself in my free time.

Development of the theme began in mid-January 2025 and was completed in February 2025. I’ve been using it exclusively for personal purposes ever since, and I made it publicly available on December 31, 2025.

You can configure the language encoding in the settings to switch the theme’s language. Currently, only Simplified Chinese and English are supported. If you want to add additional languages, you can modify the translations in i18n by updating the corresponding key-value pairs, or submit a pull request to contribute your changes.

I hope you enjoy this theme!

Demo Site：[https://demo.h4cker.zip](https://demo.h4cker.zip)

My Blog Site：[https://h4cker.zip](https://h4cker.zip)

